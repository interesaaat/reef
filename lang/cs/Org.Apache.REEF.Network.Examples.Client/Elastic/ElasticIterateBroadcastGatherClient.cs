...
{
    public class ElasticIterateBroadcastGatherClient
    {
        const string Local = "local";
        const string Yarn = "yarn";
        const string DefaultRuntimeFolder = "REEF_LOCAL_RUNTIME";

        public void RunIterateBroadcastGather(bool runOnYarn, int numTasks, int startingPortNo, int portRange)
        {
            const int numIterations = 100;
            const string driverId = "IterateGatherDriver";
            const string stage = "IterateGather";
            ...
            TestRun(merged, typeof(ElasticIterateBroadcastGatherDriver), numTasks, "IBD", runPlatform);
        }

...
{
    public class ElasticIterateScatterClient
    {
        const string Local = "local";
        const string Yarn = "yarn";
        const string DefaultRuntimeFolder = "REEF_LOCAL_RUNTIME";

        public void RunIterateScatter(bool runOnYarn, int numTasks, int startingPortNo, int portRange)
        {
            const int numIterations = 100;
            const string driverId = "ElasticIterateScatterDriver";
            const string subscription = "IterateScatter";

            IConfiguration driverConfig = TangFactory.GetTang().NewConfigurationBuilder(
                DriverConfiguration.ConfigurationModule
                    .Set(DriverConfiguration.OnDriverStarted, GenericType<ElasticIterateScatterDriver>.Class)
                    .Set(DriverConfiguration.OnEvaluatorAllocated, GenericType<ElasticIterateScatterDriver>.Class)
                    .Set(DriverConfiguration.OnEvaluatorFailed, GenericType<ElasticIterateScatterDriver>.Class)
                    .Set(DriverConfiguration.OnContextActive, GenericType<ElasticIterateScatterDriver>.Class)
                    .Set(DriverConfiguration.OnTaskRunning, GenericType<ElasticIterateScatterDriver>.Class)
                    .Set(DriverConfiguration.OnTaskCompleted, GenericType<ElasticIterateScatterDriver>.Class)
                    .Set(DriverConfiguration.OnTaskFailed, GenericType<ElasticIterateScatterDriver>.Class)
                    .Set(DriverConfiguration.OnTaskMessage, GenericType<ElasticIterateScatterDriver>.Class)

...
{
    public class ElasticParameterServerClient
    {
        const string Local = "local";
        const string Yarn = "yarn";
        const string DefaultRuntimeFolder = "REEF_LOCAL_RUNTIME";

        public void RunParameterServer(bool runOnYarn, int numTasks, int startingPortNo, int portRange)
        {
            const int numIterations = 10;
            const string driverId = "ElasticParameterServerClientDriver";

            IConfiguration driverConfig = TangFactory.GetTang().NewConfigurationBuilder(
                DriverConfiguration.ConfigurationModule
                    .Set(DriverConfiguration.OnDriverStarted, GenericType<ElasticParameterServerDriver>.Class)
                    .Set(DriverConfiguration.OnEvaluatorAllocated, GenericType<ElasticParameterServerDriver>.Class)
                    .Set(DriverConfiguration.OnEvaluatorFailed, GenericType<ElasticParameterServerDriver>.Class)
                    .Set(DriverConfiguration.OnContextActive, GenericType<ElasticParameterServerDriver>.Class)
            ...
            TestRun(merged, typeof(ElasticParameterServerDriver), numTasks, "ElasticParameterServerDriver", runPlatform);
        }

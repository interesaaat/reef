...
{
    public class ElasticIterateBroadcastGatherSummaClient
    {
        const string Local = "local";
        const string Yarn = "yarn";
        const string DefaultRuntimeFolder = "REEF_LOCAL_RUNTIME";

        public void RunIterateBroadcastGatherSumma(bool runOnYarn, int numTasks, int startingPortNo, int portRange)
        {
            const int numIterations = 100;
            const string driverId = "IterateGatherSummaDriver";
            const string subscription = "IterateGatherSumma";

            IConfiguration driverConfig = TangFactory.GetTang().NewConfigurationBuilder(
                DriverConfiguration.ConfigurationModule
                    .Set(DriverConfiguration.OnDriverStarted, GenericType<ElasticIterateBroadcastGatherSummaDriver>.Class)
                    .Set(DriverConfiguration.OnEvaluatorAllocated, GenericType<ElasticIterateBroadcastGatherSummaDriver>.Class)
                    .Set(DriverConfiguration.OnEvaluatorFailed, GenericType<ElasticIterateBroadcastGatherSummaDriver>.Class)
                    .Set(DriverConfiguration.OnContextActive, GenericType<ElasticIterateBroadcastGatherSummaDriver>.Class)
                    .Set(DriverConfiguration.OnTaskRunning, GenericType<ElasticIterateBroadcastGatherSummaDriver>.Class)
                    .Set(DriverConfiguration.OnTaskCompleted, GenericType<ElasticIterateBroadcastGatherSummaDriver>.Class)
                    .Set(DriverConfiguration.OnTaskFailed, GenericType<ElasticIterateBroadcastGatherSummaDriver>.Class)
                    .Set(DriverConfiguration.OnTaskMessage, GenericType<ElasticIterateBroadcastGatherSummaDriver>.Class)
            ...
            TestRun(merged, typeof(ElasticIterateBroadcastGatherSummaDriver), numTasks, "IBD", runPlatform);
        }
